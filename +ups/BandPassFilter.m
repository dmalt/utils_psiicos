...
function ConData = BandPassFilter(ConData, Band, TimeRange, Fsamp)
% -----------------------------------------------------------
% BandPassFilter: band-pass filter trial data, crop TimeRange
% interval and write the result into ConData cell array.
% -----------------------------------------------------------
% FORMAT:
%   ConData = BandPassFlter(ConData, Band, TimeRange, Fsamp) 
% INPUTS:
...
% OUTPUTS:
%   ConData             - {1 x N_conditions_total} cell array storing filtered
	...
	[b,a] = butter(5, Band / (Fsamp / 2));
	 for sc = 1:N_conditions_total
	    ...
	    for t = 1:size(ConData{sc}.Trials,3)
	        ...
	        Trials(:,:,t) = tmp(:,ind0:ind1);
	    end;
	    ConData{sc}.Trials = Trials;
	    sc;
	end;
